          ...
          <DataTableCell>
            <div class="flex justify-start">
              <Reserve class="text-right" reserve="100 W">
                {{ Math.round(records.averagePower()) }} W
              </Reserve>
            </div>
          </DataTableCell>
          <DataTableCell v-if="hasHeartRate(records)">
            <div class="flex justify-center">
              <Reserve class="text-right" reserve="100 bpm">
                {{ Math.round(records.averageHeartRate()) }} bpm
              </Reserve>
            </div>
          </DataTableCell>
          <DataTableCell class="text-right">
            {{ formatSeconds(records.length) }}
          </DataTableCell>
        </DataTableRow>
      </DataTableBody>
    </DataTable>
...
const hasHeartRate = (records) => {
  return records.some((record) => record.heartRate != null);
};
